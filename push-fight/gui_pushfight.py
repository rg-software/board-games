...
def what_to_draw(cell, r, c):
    if cell != Cell.LAVA and r >= 0:
        color = "gray"
        if game._state_is() == GameState.START and c > 4:
            color = "light blue"
        if game._state_is() == GameState.START and c < 5:
            color = "light salmon"
        if game._selected_is() == (r, c):
            color = "light green"
        draw_cell(r, c, color)
    if cell == Cell.LAVA:
        color = "orange"
...
def draw():
    ...
    for i in range(1, 12):
        screen.draw.line(
            ...
            (CELLSIZE * i, Game.BoardHeight * CELLSIZE + CELLSIZE * 2),
            "light gray",
        )

    for i in range(1, 6):
        screen.draw.line(
            (CELLSIZE, CELLSIZE * i + CELLSIZE),
            (Game.BoardWidth * CELLSIZE + CELLSIZE, CELLSIZE * i + CELLSIZE),
            "light gray",
        )
